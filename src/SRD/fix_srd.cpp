...
/* Authors of original stochastic rotation dynamics version in LAMMPS:
/* ----------------------------------------------------------------------
   Contributing authors: Jeremy Lechman (SNL), Pieter in 't Veld (BASF)
------------------------------------------------------------------------- */

/*  This is a modified version, authored by Jeremy C. Palmer and Renjie Chen (University of Houston), which includes:
   (i) Momentum conserving Andersen thermostat
   (ii) Collision scheme for small mesoparticles (e.g., polymer beads)
   (iii) Integration routines for small mesoparticles 
*/

/*
  Updated by T.Ikeda (Kyoto Institute Of Technology) for new ver.
*/
...
#define BIG 1.0e20
#define VBINSIZE 6                                                   //JCP: Changed from 5 to 6 in order to accomodate m, the number of small particles, in the communication buffer
...
FixSRD::FixSRD(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
  ...
  if (narg < 9) error->all(FLERR,"Illegal fix srd command");        //JCP: Changed from 8 to 9
  ...
  bigexist = 1;
  if (strcmp(arg[4],"NULL") == 0) bigexist = 0;                      //JCP: Incremented following argument indices by 
  ...
  mpcdat = 0;                                                        //JCP: Default to no MPCD Andersen thermostat
  smallexist = 0;                                                    //JCP: Default to no small particles
  mass_small = 0;                                                    //JCP: Initalize mass for small particles
  sigma_small = 0;                                                   //JCP: Initialize sigma (width of Maxwell-Boltzmann distribution) for small particles
  debugflag = 0;                                                     //JCP: Initialize debug flag

  int iarg = 8;                                                      //JCP: Changed from 8 to 9
  while (iarg < narg) {
    if (strcmp(arg[iarg],"lamda") == 0) {
      ...
      if (strcmp(arg[iarg+1],"no") == 0) 
        ...
        error->all(FLERR,"Illegal fix srd command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"debug") == 0) {                     //JCP: Added to check for debugging
        debugflag = utils::inumeric(FLERR,arg[iarg+1], false, lmp);
        iarg +=2;
    } else if (strcmp(arg[iarg],"small") == 0) {                     //JCP: Added to support group of small particles involved in rotation step
        smallexist = 1;
        time_integrate = 1;
        smallgroup = group->find(arg[iarg]);
        mass_small = utils::numeric(FLERR,arg[iarg+1], false, lmp);
        dt_smallv = update->dt;
        dt_smallf = 0.5 * update->dt * force->ftm2v; 
        iarg +=2;
    } else if (strcmp(arg[iarg],"mpcdat") == 0) {                    //JCP: Added to support MPCD Andersen Thermostat instead of standard rotation and rescaling
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix srd command");
      if (strcmp(arg[iarg+1],"no") == 0) mpcdat = 0;
      else if (strcmp(arg[iarg+1],"yes") == 0) mpcdat = 1;
      else error->all(FLERR,"Illegal fix srd command");
      iarg += 2;
    } else error->all(FLERR,"Illegal fix srd command");
  }
  ...
  if (smallexist && smallgroup < 0)                                  //JCP: Added error checking to check for small group
  ...
  if (smallexist) smallgroupbit = group->bitmask[smallgroup];        //JCP: Get small group bit
  else smallgroupbit = 0;
...
{
  int mask = 0;
  mask |= INITIAL_INTEGRATE;                                         //JCP: Added for verlocity verlet integration of small particles
  mask |= PRE_NEIGHBOR;
  mask |= POST_FORCE;
  return mask;
}
...
void FixSRD::init()
{
  ...
  Fix **fixes = modify->fix;  //T.I. for ver.23Jun2022
  ...
  if (deformflag && (tstat == 0 && mpcdat ==0) && me == 0)           //JCP: Extended to include mpcdat, which also thermostats the system
    error->warning(FLERR,
...
/* ----------------------------------------------------------------------
   JCP: First step of velocity verlet integration of small particles
------------------------------------------------------------------------- */

void FixSRD::initial_integrate(int vflag)
{
  ...
  if (smallexist) { 
    ...
    if (smallgroup == atom->firstgroup) nlocal = atom->nfirst;

    if (rmass) {
      for (int i = 0; i < nlocal; i++)
        if (mask[i] & smallgroupbit) {
          dtfm = dt_smallf / rmass[i];
          v[i][0] += dtfm * f[i][0];
          v[i][1] += dtfm * f[i][1];
          v[i][2] += dtfm * f[i][2];
          x[i][0] += dt_smallv * v[i][0];
          x[i][1] += dt_smallv * v[i][1];
          x[i][2] += dt_smallv * v[i][2];
        }

    } else {
      for (int i = 0; i < nlocal; i++)
        if (mask[i] & smallgroupbit) {
          dtfm = dt_smallf / mass[type[i]];
          v[i][0] += dtfm * f[i][0];
          v[i][1] += dtfm * f[i][1];
          v[i][2] += dtfm * f[i][2];
          x[i][0] += dt_smallv * v[i][0];
...
void FixSRD::pre_neighbor()
{
  ...
  /*
  if (reneighflag == SRD_ROTATE) {                                   //JCP:  Call new subroutine for rotation if small particles are present
    if(smallexist) {
      reset_velocities_small();
    } else{
      reset_velocities();
    }
  }
...
void FixSRD::post_force(int vflag)
{
  ...
  if (smallexist) {                                                  //JCP:  Last step of velocity verlet integration for small particles
  ...
  if (reneighflag == SRD_ROTATE) {                                   //JCP: Rotation step, moved from end of pre_neighbor;  Call new routine, reset_velocity_small, for rotation if small particles are present
    if(smallexist) {
      reset_velocities_small();
    } else{
      reset_velocities();
  ...
  nlocal = atom->nlocal;                                             //JCP:  Reset values of nlocal and nall to follow initial code .. removed declaration of nlocal since it is declared above
  int nall = nlocal + atom->nghost;
  ...
  if (bigexist) {
    flocal = f;
    tlocal = torque;
    comm->reverse_comm(this); //T.I.  For ver.23Jun2022
  }
...
void FixSRD::reset_velocities()
{
  ...
  for (i = 0; i < nbins; i++) {
    ...
    vbin[i].n = n;
    vbin[i].m = 0;                                                   //JCP: Added this for good measure, since it is now pass with vbin
...
/* ----------------------------------------------------------------------
   reset SRD velocities when small particles are present (e.g. polymers)
   may perform random shifting by up to 1/2 bin in each dimension
   called at pre-neighbor stage when all SRDs are now inside my sub-domain
   if tstat, then thermostat SRD particles as well, including streaming effects
------------------------------------------------------------------------- */
void FixSRD::reset_velocities_small()
{
  int i,j,ix,iy,iz,ibin,axis,sign,irandom;
  int n,m;                                                           //JCP:  moved n from above line, added m
  double u[3],vsum[3];
  double vsq,tbin,scale;
  double *vave,*xlamda;
  double vstream[3];
  double vransum[3];                                                 //JCP: Added this for MPCD Andersen Thermostat
  double mass_bin,mass_particle,mboltz_width;                         //JCP: added this for use in computing mass-weighted velocity and width of Maxwell-Boltzmann distribution
  ...
  for (i = 0; i < nlocal; i++)
    if ((mask[i] & groupbit) || (mask[i] & smallgroupbit )) {        //JCP: Include srd and small particles
      ix = static_cast<int> ((x[i][0]-corner[0])*bininv1x);
      ix = MAX(ix,binlo[0]);
      ix = MIN(ix,binhi[0]);
      iy = static_cast<int> ((x[i][1]-corner[1])*bininv1y);
  ...
  for (i = 0; i < nbins; i++) {
    ...
    vsum[0] = vsum[1] = vsum[2] = 0.0;
    for (j = binhead[i]; j >= 0; j = binnext[j]) {
      if (mask[j] & smallgroupbit) {                                 //JCP: Small particle
        vsum[0] += (v[j][0]*mass_small);
        vsum[1] += (v[j][1]*mass_small);
        vsum[2] += (v[j][2]*mass_small);
        m++;
      } else {                                                       //JCP: SRD particle
        vsum[0] += (v[j][0]*mass_srd);
        vsum[1] += (v[j][1]*mass_srd);
        vsum[2] += (v[j][2]*mass_srd);
        n++;
      }
    }
    ...
    vbin[i].vsum[1] = vsum[1];
    vbin[i].vsum[2] = vsum[2];
    vbin[i].n = n;                                                   //JCP: Number of SRD particles
    vbin[i].m = m;                                                   //JCP: Number of small particles 
    if (vbin[i].owner) vbin[i].random = random->uniform();
    else vbin[i].random = 0.0;
  }
  ...
  if(debugflag != 0) {                                               //JCP: Diagnostics - store linear momentum of each cell before collision
    for (i = 0; i < nbins; i++) {
      for(j = 0; j < 3; j++){
        vbin[i].linmom[j] = vbin[i].vsum[j];
      } 
    }
  }
  ...
  double tfactor = force->mvv2e / (dimension * force->boltz);        //JCP: Removed mass_srd in numerator .. included masses when computing KE / scaling
  int dof_temp = 1;
  int dof_tstat;
  if (tstat) {
    if (deformflag) dof_tstat = dof_temp = 0;
  ...
  if(mpcdat) {                                                      //JCP: If MPCD Andersen Thermostat
    ...
    for (i = 0; i < nbins; i++) {
      ...
      n = vbin[i].n;
      m = vbin[i].m;
      if ((n + m) == 0) continue;                                    //JCP: n + m, instead of just n

      vave = vbin[i].vsum;                                           //JCP: Note that vave is a pointer
      mass_bin = n*mass_srd + m*mass_small;                          //JCP: Compute total mass of SRD + small particles in bin
      vave[0] /= mass_bin;                                           //JCP: Divide by total mass of SRD + small particles in bin
      vave[1] /= mass_bin;                                           //JCP: " "
      vave[2] /= mass_bin;                                           //JCP: " "

      vransum[0] = vransum[1] = vransum[2] = 0.0;
      for (j = binhead[i]; j >= 0; j = binnext[j]) {                 //JCP: Draw new velocities from Maxwell-Boltzmann distribution
        mboltz_width = sigma;
        if (mask[j] & smallgroupbit) mboltz_width = sigma_small; 
//        while(1){
     	  v[j][0] = mboltz_width * random->gaussian();
	  v[j][1] = mboltz_width * random->gaussian();
	  v[j][2] = mboltz_width * random->gaussian();
	  vsq = v[j][0]*v[j][0] + v[j][1]*v[j][1] + v[j][2]*v[j][2];
//	  if (vsq <= vmaxsq) break;
//	}

        mass_particle = mass_srd;                                    //JCP: Compute linear momentum components from new random velocities
        if (mask[j] & smallgroupbit) mass_particle = mass_small;
        vransum[0] += (v[j][0]*mass_particle);
	vransum[1] += (v[j][1]*mass_particle);
	vransum[2] += (v[j][2]*mass_particle);
      }
      vbin[i].value[0] = vransum[0];
      vbin[i].value[1] = vransum[1];
      vbin[i].value[2] = vransum[2];
    }

    if (shifts[shiftflag].commflag) xbin_comm(shiftflag,3);          //JCP: Sum linear momentum components across processors that have particles in the cell
    ...
    for (i = 0; i < nbins; i++) {
      ...
      mass_bin = n*mass_srd + m*mass_small;                          //JCP: Compute mass-weighted average of new random velocities
      vransum[0] = vbin[i].value[0]/mass_bin;
      vransum[1] = vbin[i].value[1]/mass_bin;
      vransum[2] = vbin[i].value[2]/mass_bin;

      vsq = 0.0;
      for (j = binhead[i]; j >= 0; j = binnext[j]) {

        u[0] = v[j][0] - vransum[0];                                 //JCP: Subtract mass-weighted average of new random velocities from each particle's new velocity
        u[1] = v[j][1] - vransum[1];
        u[2] = v[j][2] - vransum[2];
         
        mass_particle = mass_srd;                                    //JCP: Compute kinetic energy (wrt to cell's average, vavg) for temperature calculation (later)
        if (mask[j] & smallgroupbit) mass_particle = mass_small;     
        vsq += (u[0]*u[0] + u[1]*u[1] + u[2]*u[2])*mass_particle;   
          
        if (deformflag) {                                            //JCP:  Add cell's average velocity, or stream velocity if deforming box
          xlamda = vbin[i].xctr;
          vstream[0] = h_rate[0]*xlamda[0] + h_rate[5]*xlamda[1] +
            h_rate[4]*xlamda[2] + h_ratelo[0];
          vstream[1] = h_rate[1]*xlamda[1] + h_rate[3]*xlamda[2] + h_ratelo[1];
        ...
        v[j][0] =  u[0] + vstream[0];                                //JCP:  Add cell's average velocity / stream velocity
        v[j][1] =  u[1] + vstream[1];
        v[j][2] =  u[2] + vstream[2];
      }
      vbin[i].value[0] = vsq;                                        //JCP:  Kinetic energy for cell temperature calculation 
    }
      
    if (shifts[shiftflag].commflag) xbin_comm(shiftflag,1);         


  } else {                                                           //JCP: Else standard SRD
    ...
    for (i = 0; i < nbins; i++) {
      ...
      for (j = binhead[i]; j >= 0; j = binnext[j]) {   
        ...
        mass_particle = mass_srd;                                    //JCP: Default to SRD particle's mass                               
        if (mask[j] & smallgroupbit) mass_particle = mass_small;     //JCP: If small particle, reset mass    
        vsq += (u[0]*u[0] + u[1]*u[1] + u[2]*u[2])*mass_particle;    //JCP: Multiply the particle mass times v^2 
        v[j][0] = u[0] + vave[0];
        v[j][1] = u[1] + vave[1];
        v[j][2] = u[2] + vave[2];
      ...
      if ((n + m) > 1) vbin[i].value[0] = vsq;                       //JCP: n + m instead of just n
    }
    ...
    if (tstat) {
      for (i = 0; i < nbins; i++){
        n = vbin[i].n;
        m = vbin[i].m;                                               //JCP: number of small particles
        if ((n + m) <= 1) continue;                                  //JCP: n + m instead of just n
      
        // vsum is already average velocity                          //JCP: Note that vsum here is the center of mass velocity, b/c vave above is a pointer, hence its already been weighted and divided my particle masses
        vave = vbin[i].vsum;
        ...
        tbin = vbin[i].value[0]/(n + m - dof_tstat) * tfactor;       //JCP: n + m instead of just m
        ...
        for (j = binhead[i]; j >= 0; j = binnext[j]) {	
          ...
          mass_particle = mass_srd;                                  //JCP:  Default to SRD particle's mass, else reset mass if small particle
          if (mask[j] & smallgroupbit) mass_particle = mass_small;
          vsq += (u[0]*u[0] + u[1]*u[1] + u[2]*u[2])*mass_particle;
          v[j][0] = u[0] + vstream[0];
          v[j][1] = u[1] + vstream[1];
        ...
        vbin[i].value[0] = vsq;                                       //JCP:  Kinetic energy for cell temperature calculatio
      }

    if (shifts[shiftflag].commflag) xbin_comm(shiftflag,1);
    }

  }                                                                  //JCP: End MPCDAT / SRD  if-else


  for (i = 0; i < nbins; i++){                                       //JCP:  Not sure if this is correct
    if (vbin[i].owner) {
      if ((vbin[i].n + vbin[i].m)  > 1) {                            //JCP: n + m instead of just n
	srd_bin_temp += vbin[i].value[0]/(vbin[i].n + vbin[i].m - dof_temp); //JCP: added m dof calculation
	srd_bin_count++;
      }
    }
  }
  ...
  if(debugflag != 0) {                                               //JCP: Added this block for diagnostics - computes final linear momentum for each cell
    for (i = 0; i < nbins; i++) {
      n = 0;
      m = 0;
      vsum[0] = vsum[1] = vsum[2] = 0.0;
  ...
  // fprintf(screen,"Debug: Me %d nbins %d, mass_small %g\n",me,nbins,mass_small);

   for (i = 0; i < nbins; i++) {                                     //JCP: Compute difference between initial and final linear moment, check if nonzero
      for(j = 0; j < 3; j++){
        if(me == 0) {
          if(debugflag == 1){
            if(fabs(vbin[i].vsum[j]-vbin[i].linmom[j]) > 1.0E-13) {
             fprintf(screen,"Debug: Me %d Bin %d, srd %d, small, %d, component %d, initial %g, final %g,  diff: %g\n",me,i,vbin[i].n,vbin[i].m,j,vbin[i].linmom[j],vbin[i].vsum[j],vbin[i].vsum[j]-vbin[i].linmom[j]);
            }
          } else {
           fprintf(screen,"Debug: Me %d Bin %d, srd %d, small, %d, component %d, initial %g, final %g,  diff: %g\n",me,i,vbin[i].n,vbin[i].m,j,vbin[i].linmom[j],vbin[i].vsum[j],vbin[i].vsum[j]-vbin[i].linmom[j]);
          }
...
void FixSRD::vbin_pack(BinAve *vbin, int n, int *list, double *buf)
{
  ...
  for (int i = 0; i < n; i++) {
    j = list[i];
    buf[m++] = vbin[j].n;
    buf[m++] = vbin[j].m;                                             //JCP: Added this for communicating # of small particles
    buf[m++] = vbin[j].vsum[0];
    buf[m++] = vbin[j].vsum[1];
    buf[m++] = vbin[j].vsum[2];
    buf[m++] = vbin[j].random;
...
void FixSRD::vbin_unpack(double *buf, BinAve *vbin, int n, int *list)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    vbin[j].m += static_cast<int> (buf[m++]);                        //JCP: Added this for communicating # of small particles
    vbin[j].vsum[0] += buf[m++];
    vbin[j].vsum[1] += buf[m++];
    vbin[j].vsum[2] += buf[m++];
    vbin[j].random += buf[m++];
...
void FixSRD::parameterize()
{
  ...
  fprintf(screen,"nevery, dt_srd, dt_big %d %g %g  \n" ,nevery,dt_srd,dt_big);
  ...
  flag = 0;                                                          //JCP: Added this block to ensure that all small particles have the same mass (copied from above)
 
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & smallgroupbit) {
      if (rmass) {
         if (rmass[i] != mass_small) flag = 1;
      } else {
         if (mass[type[i]] != mass_small) flag = 1;
      }
    }

  MPI_Allreduce(&flag,&flagall,1,MPI_INT,MPI_MAX,world);
  if (flagall)
    error->all(FLERR,"Fix srd requires small particles all have same mass");

  sigma_small = 0.0;                                                 //JCP: Compute sigma, width of Maxwell-Botlzmann distribution, for small particles
  if(mass_small != 0.0) sigma_small = sqrt(force->boltz*temperature_srd/mass_small/force->mvv2e);

  if(debugflag) printf("Debug: Proc # %d  mass_small %g sigma_small %g  mass_srd %g \n",me,mass_small, sigma_small, mass_srd);
  ...
  bigint nsmall = 0;
  if (smallexist) nsmall = group->count(smallgroup);                 //JCP: Added to counter number of small particles
  ...
  double mass_big_avg = 0.0;                                         //JCP: Added to compute average mass per big particle
  if (bigexist && mbig != 0) mass_big_avg = (double) mass_big / mbig; 
  ...
  if (dimension != 3) error->all(FLERR,"This version of SRD only supports 3 dimesional systems");   //JCP: Added this to avoid specifiy lamda until it can be re-tested

  double viscosity;                                                  //JCP: Added diffusivity and Schmidt
  double diffusivity;
  double density;
  double schmidt; 
  if (dimension == 3) {
    ...
    diffusivity = ((force->boltz*temperature_srd*dt_srd)/(2.0*mass_srd*force->mvv2e))*((3.0*srd_per_cell)/(srd_per_cell - 1.0 + exp(-srd_per_cell)) - 1.0);

    schmidt = viscosity/diffusivity;

    fprintf(screen,"gridsrd, dt_srd, srd_per_cell, kT, mass_srd, mmv2e %g %g %g %g %g %g \n" ,gridsrd, dt_srd,srd_per_cell, force->boltz*temperature_srd, mass_srd,force->mvv2e);
  ...
  if(mpcdat) {                                                       //JCP: Added for MPCD Andersen Thermostat
 
     viscosity = gridsrd*gridsrd*(srd_per_cell - 1.0 + exp(-srd_per_cell))/(12.0*srd_per_cell*dt_srd);  //JCP: Collisional term
     viscosity += ((force->boltz*temperature_srd*dt_srd)/(mass_srd*force->mvv2e))*((srd_per_cell)/(srd_per_cell - 1.0 + exp(-srd_per_cell)) - 0.5);  //JCP: Kinetic term
    
     //viscosity = ((srd_per_cell*force->boltz*temperature_srd*dt_srd)/(gridsrd*gridsrd*gridsrd))*(srd_per_cell/(srd_per_cell - 1.0 + exp(-srd_per_cell)) - 0.5); 
     //viscosity += (mass_srd*force->mvv2e)*(srd_per_cell - 1.0 + exp(-srd_per_cell))/(12.0*gridsrd*dt_srd);
     diffusivity = ((force->boltz*temperature_srd*dt_srd)/(mass_srd*force->mvv2e))*(srd_per_cell/(srd_per_cell - 1.0 + exp(-srd_per_cell)) - 0.5); 

     schmidt = viscosity/diffusivity;

  }

  density = srd_per_cell*(mass_srd*force->mvv2e)/(gridsrd*gridsrd*gridsrd);    //JCP: !!! Re-check units !!!
  viscosity *= (force->xxt2kmu)*density;                                       //JCP: Convert to cm^2/sec units, divide by density to get dynamic
  diffusivity *= force->xxt2kmu;

  // print SRD parameters

  if (me == 0) {
    if (screen) {
      fprintf(screen,"SRD info:\n");                                 //JCP: modified 2nd and 3rd to dump number of small particles, mass of each particle 
      fprintf(screen,
              "  SRD/small/big particles = " BIGINT_FORMAT " " BIGINT_FORMAT " " BIGINT_FORMAT "\n",
              nsrd,nsmall,mbig);
      fprintf(screen,"  SRD/small/big mass = %g %g %g \n" ,mass_srd,mass_small,mass_big_avg);  
      ...
      fprintf(screen,"  SRD dynamic viscosity, diffusivity, Schmidt = %g,  %g,  %g\n",viscosity, diffusivity, schmidt);
    ...
    if (logfile) {
      ...
      fprintf(logfile,
              "  SRD/small/big particles = " BIGINT_FORMAT " " BIGINT_FORMAT " " BIGINT_FORMAT "\n",
              nsrd,nsmall,mbig);
      fprintf(logfile,"  SRD/small/big mass = %g %g %g \n" ,mass_srd,mass_small,mass_big_avg);
      ...
      fprintf(logfile,"  SRD dynamic viscosity, diffusivity, Schmidt = %g,  %g,  %g\n",viscosity, diffusivity, schmidt);
...
void FixSRD::setup_velocity_shift(int ishift, int dynamic)
{
  ...
  if ((tstat && deformflag) || (mpcdat && deformflag) ) {             //JCP: Added case for MPCDAT and box deformation
    m = 0;
    for (k = 0; k < nbinz; k++)
      for (j = 0; j < nbiny; j++)
        for (i = 0; i < nbinx; i++) {
