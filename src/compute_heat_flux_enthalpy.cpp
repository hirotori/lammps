...
#include "compute_heat_flux_enthalpy.h"
...
ComputeHeatFluxEnthalpy::ComputeHeatFluxEnthalpy(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg),
  id_ke(nullptr), id_pe(nullptr), id_stress(nullptr)
{
  if (narg != 6) error->all(FLERR,"Illegal compute heat/flux/enthalpy command");
  ...
  if (ike < 0 || ipe < 0 || istress < 0)
    error->all(FLERR,"Could not find compute heat/flux/enthalpy compute ID");
  if (strcmp(modify->compute[ike]->style,"ke/atom") != 0)
    error->all(FLERR,"Compute heat/flux/enthalpy compute ID does not compute ke/atom");
  if (modify->compute[ipe]->peatomflag == 0)
    error->all(FLERR,"Compute heat/flux/enthalpy compute ID does not compute pe/atom");
  if (modify->compute[istress]->pressatomflag != 1
      && modify->compute[istress]->pressatomflag != 2)
    error->all(FLERR,
               "Compute heat/flux/enthalpy compute ID does not compute stress/atom or centroid/stress/atom");
...
ComputeHeatFluxEnthalpy::~ComputeHeatFluxEnthalpy()
{
  delete [] id_ke;
  delete [] id_pe;
  delete [] id_stress;
...
void ComputeHeatFluxEnthalpy::init()
{
  // error checks
...
void ComputeHeatFluxEnthalpy::compute_vector()
{
  ...
  // 3rd 3 terms are total heat flux including enthalpy portion
